...
public class PedestrianObject : MonoBehaviour 
{
	...
	void FixedUpdate ()
	{
        ...
        Disaster closest_disaster = getClosestDisaster();
        if (closest_disaster == null)
            followPedestrianPaths();
        else
            avoidDisaster(closest_disaster);
        
       
	}

    private Disaster getClosestDisaster()
    {
        Disaster[] disasters = FindObjectsOfType<Disaster>();
        Vector3 closest_disaster_to_pedestrian_vector = Vector3.zero;
        Disaster closest_disaster = null;
        // Find the closest disaster
        for (int i = 0; i < disasters.Length; i++)
        {
            Vector3 current_disaster_vector = (gameObject.transform.position - disasters[i].transform.position);
            // Base case (first time)
            if (closest_disaster_to_pedestrian_vector == Vector3.zero)
            {
                closest_disaster_to_pedestrian_vector = current_disaster_vector;
                closest_disaster = disasters[i];
            }
            else
            {
                if (closest_disaster_to_pedestrian_vector.magnitude > current_disaster_vector.magnitude)
                {
                    closest_disaster_to_pedestrian_vector = current_disaster_vector;
                    closest_disaster = disasters[i];
                }
